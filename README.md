...
源码、测试、构建配置和开发文档位于私有仓库：[ps-slicer-source](https://github.com/Homer79980/ps-slicer-source)。公开下载仓库不提供源码，也不授予开源许可。
...
以下视频来自本次 Photoshop 实机操作录屏，统一使用 9 个食物图标素材，展示从选层、分析、校对到导出的完整工作流。视频原文件保留在仓库的 [`docs/videos`](docs/videos/) 目录中；浏览器不能直接播放时，可点击链接下载。
...
这是有意的分发方式：公开仓库只提供 CCX 下载入口、使用文档、配图和录屏；源码私库单独维护。
